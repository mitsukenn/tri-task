...
async function load() {
  try {
    const result = await window.storage.get(`satisfy-${TODAY_KEY}`, false);
...
async function save(d) {
  try {
    await window.storage.set(`satisfy-${TODAY_KEY}`, JSON.stringify(d), false);
  } catch {}
}

async function loadHistory() {
  try {
    const result = await window.storage.list("satisfy-", false);
    if (!result || !result.keys) return [];
    
    const entries = [];
    for (const key of result.keys) {
      if (key !== `satisfy-${TODAY_KEY}`) {
        try {
          const data = await window.storage.get(key, false);
          if (data) {
            entries.push({
              date: key.replace("satisfy-", ""),
...
export default function App() {
  ...
  useEffect(() => {
    if (showHist) {
      loadHistory().then(setHistEntries);
    }
  }, [showHist]);
